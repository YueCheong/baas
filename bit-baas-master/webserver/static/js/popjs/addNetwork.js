...
layui.use(['form'], function(){
    ...
    //表单取值
    layui.$('#add-network-sure-btn').on('click', function(){
        var data = form.val('add-network');
        // data.OrdererOrg = "{" + data.OrdererOrg + "}";
        api('../api/networks/' + data["Name"], "", 'put').then(resove => {
            if(resove > 299) {
                alert("发生错误");
            }
            else {
    ...
    layui.$('#add-network-cancel-btn').on('click', function(){
        var index = parent.layer.index; //获取当前弹层的索引号
        parent.layer.close(index); //关闭当前弹层
    });
});
